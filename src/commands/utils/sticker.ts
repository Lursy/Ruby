...
export const makeStiker = async (mmedia: proto.Message.IVideoMessage | proto.Message.IImageMessage | proto.Message.IDocumentMessage, type: string = "0") => {
  ...
  if(mmedia.mimetype.split("/")[0] !== "image" || type === "1"){
    try{
...
export const toIMG = async(sticker: proto.Message.IStickerMessage, ext: keyof FormatEnum) => {
  let media = await downloadContentFromMessage(sticker, "sticker");
  ...
  return sharp(buffer).toFormat(ext).toBuffer();
